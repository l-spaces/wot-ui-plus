...
<script lang="ts" setup>
  ...
  watch(
    ...
    (newValue) => {
      if (!isNumber(newValue) && !isString(newValue)) {
        console.error('[wot ui plus] error(wd-tabs): the type of value should be number or string')
      }
      // 保证不为非空字符串，小于0的数字
      if (newValue === '' || !isDef(newValue)) {
        console.error('[wot ui plus] error(wd-tabs): tabs.s value cannot be null or undefined')
      }
      if (typeof newValue === 'number' && newValue < 0) {
        console.error('[wot ui plus] error(wd-tabs): tabs.s value cannot be less than zero')
      }
    },
    {
      immediate: true,
  ...
  async function updateLineStyle(animation: boolean = true) {
    ...
    try {
      ...
      console.error('[wot ui plus] error(wd-tabs): update line style failed', error)
    }
  }
  ...
  function getActiveIndex(value: number | string) {
    ...
    if (isNumber(value) && value >= children.length) {
      ...
      console.error('[wot ui plus] warning(wd-tabs): the type of tabs value is Number should not be less than its children')
      value = 0
    }
    // 如果是字符串直接匹配，匹配不到用0兜底
    if (isString(value)) {
