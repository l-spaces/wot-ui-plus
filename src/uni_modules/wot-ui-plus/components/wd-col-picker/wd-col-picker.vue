  ...
  watch(
    ...
    (newValue, oldValue) => {
      if (newValue.length && !isArray(newValue[0])) {
        console.error('[wot ui plus] error(wd-col-picker): the columns props of wd-col-picker should be a two-dimensional array')
        return
      }
      ...
      columnChange({
        ...
        resolve: (nextColumn: Record<string, any>[]) => {
          if (!isArray(nextColumn)) {
            console.error('[wot ui plus] error(wd-col-picker): the data of each column of wd-col-picker should be an array')
            return
          }
