<template>
  <view :class="rootClass" :id="sliderButtonId" :style="rootStyle">
    <!-- 背景提示文字 -->
    ...
    <view class="wd-slider-button__track" :style="trackStyle">
      ...
      <!-- 滑块 -->
      <view class="wd-slider-button__button" @touchstart.prevent="onTouchStart" @touchmove.prevent="onTouchMove" @touchend="onTouchEnd">
        <slot v-if="isPass" name="success-icon">
          <wd-icon custom-class="wd-slider-button__button-icon wd-slider-button__button-icon--success" :name="successIcon" :size="successIconSize" />
        </slot>

        <slot v-else name="icon">
          <wd-icon custom-class="wd-slider-button__button-icon" :name="icon" :size="iconSize" />
        </slot>
      </view>
    </view>
  </view>
...
<script lang="ts" setup>
  ...
  import { sliderButtonProps, type SliderButtonEmits, type SliderButtonExpose } from './types'
  ...
  const sliderButtonId = ref<string>(`wd-slider-button-${uuid()}`)
  ...
  const init = async () => {
    try {
      const rect = await getRect(`#${sliderButtonId.value}`, false, proxy)
  ...
  defineExpose<SliderButtonExpose>({ reset, init })
</script>
