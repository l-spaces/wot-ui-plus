        ...
        <view class="wd-slider__bar" :style="barStyle">
          <template v-if="isRange">
            ...
            <view
              ...
              @touchstart="(event: any) => onSliderTouchStart(event, 0)"
            ...
            <view
              ...
              :style="sliderButtonStyle"
              @touchstart="(event: any) => onSliderTouchStart(event, 1)"
              @touchmove="onSliderTouchMove"
              @touchend="onSliderTouchEnd"
              @touchcancel="onSliderTouchEnd"
            >
...
<script lang="ts" setup>
  ...
  const stepProp = computed(() => {
    if (props.step <= 0) {
      console.warn('[wot ui plus] warning(wd-slider): step must be greater than 0')
      return 1
    }
    return props.step
  })
  ...
  function normalizeRangeValues(value: number[]): [number, number] {
    ...
    if (!Array.isArray(value) || value.length < 2) {
      console.warn('[wot ui plus] warning(wd-slider): range value should be an array with at least 2 elements')
