<template>
  <page-wraper>
    <demo-block title="基础用法">
      ...
      <br />
      <wd-sort-button v-model="value1" filled title="价格" @change="handleChange1" />
    </demo-block>

    <demo-block title="下划线颜色 line-color='red'">
      <wd-sort-button v-model="value2" line-color="red" title="价格" @change="handleChange1" />
    </demo-block>

    <demo-block title="设置 allow-reset 允许重置按钮">
      <wd-sort-button v-model="value3" title="价格" allow-reset @change="handleChange2" />
    </demo-block>

    <demo-block title="设置 desc-first 优先切换为降序">
      <wd-sort-button v-model="value4" title="价格" desc-first @change="handleChange3" />
    </demo-block>

    <demo-block title="不展示下划线（当只有一个排序按钮时，应取消展示下划线）">
      <wd-sort-button v-model="value5" title="价格" :line="false" @change="handleChange4" />
    </demo-block>
  </page-wraper>
</template>
