 ...
 * 广告数据加载组合式函数
 * 
 * 该函数提供了一套完整的广告数据获取机制，支持多数据源故障转移，
 * 组件挂载时自动从配置的数据源获取广告数据。返回响应式的广告数据状态，
 * 可在组件中直接使用。
 * 
 * @returns {Object} 返回包含响应式广告数据的对象
 * @returns {Ref<AdData[]>} returns.data - 响应式的广告数据数组
 * @throws {Error} 当所有数据源都加载失败时，内部会捕获错误并返回空数组
 * @example
 * // 在Vue组件中使用
