...
const href = computed(() => {
  ...
  const tab = kebabToCamel(paths[paths.length - 1])
  if(tab === 'intro') return baseUrl.value
...
.demo-model {
  ...
  z-index: 10;
  right: 32px;
  top: calc(var(--vp-nav-height) + var(--vp-layout-top-height, 0px) + 50px);
  width: 330px;
  font-size: 16px;
  ...
  transition: all 0.3s ease-in-out;
	/* background-image: url(./../../../public/iPhone13.png); */
