...
const oldVersion = JSON.parse(readFileSync(path.resolve(__dirname, '../package.json'), 'utf-8')).version
// 定义文档中需要被替换的最低版本占位符
...
inquirer
  ...
  .then((answers: any) => {
    ...
    const tarfetPackageJson = JSON.parse(readFileSync(path.resolve(__dirname, '../src/uni_modules/wot-ui-plus/package.json'), 'utf-8'))
