...
- `outdated`: 检查过时的依赖包。
- `store:prune`: 清理 pnpm 存储缓存。

### 代码质量检查

- `type-check`: 运行 TypeScript 类型检查。
- `lint`: 运行 ESLint 检查。
- `lint:fix`: 运行 ESLint 检查并自动修复问题。
- `format:check`: 使用 Prettier 检查代码格式。
- `format`: 使用 Prettier 格式化代码。
- `lint:format`: 运行 ESLint 修复和 Prettier 格式化。
- `quotes:check`: 检查引号使用规范。
- `quotes:fix`: 自动修复引号使用问题。
- `quotes:check-all`: 全面检查引号和格式。
